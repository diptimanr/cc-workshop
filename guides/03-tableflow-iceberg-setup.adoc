= 🗄️ Tableflow & Iceberg Setup with DuckDB Analytics (25 minutes)
...
# Replace <CLUSTER_ID> with your actual Kafka cluster ID (lkc-XXXXXX)
...
confluent tableflow topic describe crypto-prices --cluster <CLUSTER_ID>

# Wait for initial data to be materialized (2-3 minutes)
confluent tableflow topic list --cluster <CLUSTER_ID>
----
...
DESCRIBE iceberg_catalog."lkc-XXXXXX"."crypto-prices";

-- Query the crypto-prices data via DuckDB
SELECT * FROM iceberg_catalog."lkc-XXXXXX"."crypto-prices" LIMIT 10;
...
FROM iceberg_catalog."lkc-XXXXXX"."crypto-prices"
...
CREATE SECRET iceberg_secret (
    ...
    ENDPOINT 'https://tableflow.us-west-2.aws.confluent.cloud/iceberg/catalog/organizations/<UUID>/environments/<env-ID>',
    OAUTH2_SCOPE 'catalog'
);
----
...
SHOW TABLES FROM iceberg_catalog."lkc-XXXXXX";

-- Check if table name matches exactly (topic name with hyphens)
SELECT * FROM iceberg_catalog."lkc-XXXXXX"."crypto-prices" LIMIT 1;
----
...
**Next**: Proceed to `04-flink-hands-on.adoc` for stream processing that will create additional tables exposed via Tableflow.
