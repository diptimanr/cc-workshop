...
# DO NOT CREATE THE REST OF THE TOPICS EXCEPT 'crypto-prices' NOW
...
# Windows - Create topic for cryptocurrency prices
confluent kafka topic create crypto-prices `
  --partitions 3 `
  --config retention.ms=604800000 `
  --config cleanup.policy=delete

# Windows - Create topic for price alerts (smaller retention)
confluent kafka topic create price-alerts `
  --partitions 1 `
  --config retention.ms=86400000 `
  --config cleanup.policy=delete

# Windows - Create compacted topic for latest prices
confluent kafka topic create latest-prices `
  --partitions 3 `
  --config cleanup.policy=compact `
  --config min.cleanable.dirty.ratio=0.01  
...
# Windows - Update topic configuration (example: increase retention)
confluent kafka topic update crypto-prices `
  --config retention.ms=1209600000
...
# Windows :
confluent connect cluster create --config-file .\configs\connector-configs\http-source-coingecko.json
----
