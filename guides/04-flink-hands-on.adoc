...
**Kafka Topics → Flink SQL Processing → Multiple Output Topics → Tableflow Materialization**. 
...
# Windows - Create a compute pool for Flink processing
confluent flink compute-pool create cc-workshop-pool `
  --cloud aws `
  --region us-east-1 `
  --max-cfu 5 `
  --environment env-9g8pkm
...
-- Windows CMD prompt - Compare current prices across cryptocurrencies
SELECT 
  coin_id,
  usd as current_price,
  usd_24h_change as daily_change,
...
# Windows - Enable Tableflow for the price-alerts topic created by Flink
confluent tableflow topic enable price-alerts `
  --cluster $CC_KAFKA_CLUSTER `
  --storage-type MANAGED `
  --table-formats ICEBERG `
...
# Windows - Enable Tableflow for the crypto-trends topic created by Flink
confluent tableflow topic enable crypto-trends `
  --cluster $CC_KAFKA_CLUSTER `
  --storage-type MANAGED `
  --table-formats ICEBERG `
