= 🌊 Flink Stream Processing & Tableflow Integration (45 minutes)
...
* ✅ Created and configured a Flink compute pool
* ✅ Registered Kafka topics as Flink tables
* ✅ Transformed nested cryptocurrency data into exploded format
* ✅ Written SQL queries for real-time cryptocurrency analysis
* ✅ Created derived tables (price-alerts, crypto-trends) for Tableflow
* ✅ Materialized Flink outputs as Iceberg tables via Tableflow
* ✅ Queried Flink-created tables through DuckDB
...
== 🏗️ Flink Environment Setup (10 minutes)
...
== 🔄 Data Transformation: Exploding Nested Cryptocurrency Data (10 minutes)

=== Create Exploded Cryptocurrency Table

The first step in our stream processing pipeline is to transform the nested cryptocurrency data from the CoinGecko API into individual records for each coin. This "explosion" operation makes the data easier to work with in subsequent analytics.
...
=== Create Derived Streams for Trends
...
== 🗄️ Tableflow Integration (10 minutes)
...
SELECT * FROM iceberg_catalog."lkc-XXXXXX"."price-alerts" 
...
FROM iceberg_catalog."lkc-XXXXXX"."price-alerts"
...
**Next**: Proceed to `05-advanced-analytics.adoc` for advanced analytics and workshop completion.
